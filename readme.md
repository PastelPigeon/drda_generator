...
| **脸图**   | `[face=spr_face_susie_alt_0]说话[/face]`                       | 指定这段文字出现时显示的角色头像。<br>参数是头像文件名（不加路径和后缀）。<br>（素材包在 `assets/character_faces/` 或 [下载链接](https://wwjo.lanzouu.com/ipGkz31vx0pe)） | **核心功能！** 设置对话时使用的脸图                                      |
...
| **小对话框（画外音）**   | `[off_screen face=example text=example]`                                  | 在**当前位置**插入一个小对话框。<br>face属性需填写脸图id，text属性填写对话文本（不支持bbcode标签）</br>**注意**：当前版本建议放在需要等待的位置的**前一个字符处**。 | **非常重要！** 控制对话节奏，避免文字瞬间显示完。                      |
