extends "res://generator/scripts/generator_preview/generator_preview.gd"
...
func _update_ui():
	...
	# 判断bbcode文本是否有错误，有错误直接返回，不更新
	if EditorBbcodeParser.check_bbcode_string(content) != EditorBbcodeParser.BbcodeStringError.NONE:
		return
	
	# 更新预览动画
	update_preview_animation([content], OptionsConfigManager.get_options())
