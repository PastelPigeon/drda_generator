...
func _on_text_submitted(new_text: String):
	...
	# 设置资产属性
	ExternalAssetsManager.set_asset_attribute_CHARACTER_SOUNDS(AssetsManagerDataManager.selected_key, AssetsManagerDataManager.selected_index, ExternalAssetsManager.get_asset_info(AssetsManagerDataManager.selected_asset_type, AssetsManagerDataManager.selected_key, AssetsManagerDataManager.selected_index)["sound_timer"], float(new_text))
...
func _update_ui():
	...
	if AssetsManagerDataManager.selected_key != "" and AssetsManagerDataManager.selected_index != -1 and AssetsManagerDataManager.selected_asset_type == ExternalAssetsManager.AssetType.CHARACTER_SOUNDS:
		text = str(ExternalAssetsManager.get_asset_info(AssetsManagerDataManager.selected_asset_type, AssetsManagerDataManager.selected_key, AssetsManagerDataManager.selected_index)["random_pitch"])
